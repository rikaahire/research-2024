...
def guess_top_5(sims, analogy, analogy_type, iw, wi, word_a, word_b, word_c):
    sim_a = sims[analogy[analogy_type]["wi"][word_a]]
    sim_b = sims[analogy[analogy_type]["wi"][word_b]]
    sim_c = sims[analogy[analogy_type]["wi"][word_c]]
    ...
    add_sim[wi[word_b]] = 0
    add_sim[wi[word_c]] = 0
    guess_add_indices = np.argpartition(add_sim, -5)[-5:]
    guess_add_indices = guess_add_indices[np.argsort(-add_sim[guess_add_indices])]
    guess_add_top_5 = [iw[i] for i in guess_add_indices]

    mul_sim = sim_b * sim_c * np.reciprocal(sim_a + 0.01)
    mul_sim[wi[word_a]] = 0
    mul_sim[wi[word_b]] = 0
    mul_sim[wi[word_c]] = 0
    guess_mul_indices = np.argpartition(mul_sim, -5)[-5:]
    guess_mul_indices = guess_mul_indices[np.argsort(-mul_sim[guess_mul_indices])]
    guess_mul_top_5 = [iw[i] for i in guess_mul_indices]

    return guess_add_top_5, guess_mul_top_5
...
def main():
    ...
    for analogy_type in analogy.keys():  # Calculate the accuracy for each relation type
        ...
        for question in analogy[analogy_type]["questions"]:  # Loop for each analogy question
            ...
            #if guess_mul == word_d:
                #correct_mul_num += 1
            guess_add_top_5, guess_mul_top_5 = guess_top_5(sims, analogy, analogy_type, iw, wi, word_a, word_b, word_c)
            if word_d in guess_add_top_5:
                correct_add_num += 1
            if word_d in guess_mul_top_5:
                correct_mul_num += 1
        ...
        if analogy[analogy_type]["seen"] == 0:
            ...
            output.append(analogy_type + " add/mul: " + str(round(0.0, 3)) + "/" + str(round(0.0, 3)))
        else:
            ...
            output.append(analogy_type + " add/mul: " + str(round(acc_add, 3)) + "/" + str(round(acc_mul, 3)))
        # Store the results
    ...
    if seen == 0:
        ...
        output.append("Total accuracy (add): " + str(round(0.0, 3)))
        output.append("Total accuracy (mul): " + str(round(0.0, 3)))
    else:
        ...
        output.append("Total accuracy (add): " + str(round(float(correct_add_num)/seen, 3)))
        output.append("Total accuracy (mul): " + str(round(float(correct_mul_num)/seen, 3)))

    # Determine the filename
    base_name = os.path.basename(vectors_path)
    file_prefix = base_name.split('_')[-1].split('.')[0]
    
    analogy_prefix = os.path.basename(analogy_path).split('.')[0]
    filename = f"{analogy_prefix}_{file_prefix}.txt"
