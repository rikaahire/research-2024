...
with open('/scratch/network/sa3937/wordembed/w2v_dec_nopunc/correlation_value.txt', 'w') as f_out:
    ...
    for word_list_1, word_list_2, label in word_pairs:
        ...
        if result_1 is not None and result_2 is not None:
            ...
            f_out.write(f"{label}:\n")
            f_out.write(f"Pearson correlation coefficient: {corr}\n")
            ...
            plt.scatter(diff_dist_1, diff_dist_2)
            plt.xlabel(f'Difference in distances in 1970s ({label})')
            plt.ylabel(f'Difference in distances in 2000s ({label})')
            plt.title(f'Correlation of differences between {label}')
            plt.plot(diff_dist_1, intercept + slope * diff_dist_1, 'r')
            plt.savefig(f'correlation_value_{label.replace(" ", "_")}.png')
            plt.show()
